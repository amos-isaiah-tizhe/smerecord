...
const getUserDetail = async (req, res) => {
  try {
    ...
    const [statsAgg, recentTxs, bookCount, categoryCount] =
      await Promise.all([
        ...
        Transaction.find({
          ...
          .select('amount type date category paymentMethod')
          .sort({ date: -1 })
          .limit(10)
          .lean(),

        RecordBook.countDocuments({
          userId,
          isArchived: false
        }),
...
const getAllTransactions = async (req, res) => {
  try {
    const { type, paymentMethod, from, to, search, page = 1, limit = 30 } = req.query;
    ...
    if (search) {
      ...
      txs = txs.filter(t =>
        ...
        (t.userId?.businessName || '').toLowerCase().includes(q) ||
