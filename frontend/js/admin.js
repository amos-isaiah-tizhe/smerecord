...
function attachAdminEventHandlers() {
  ...
  document.getElementById('filterTxType')?.addEventListener('change', renderAdminTxTable);
  document.getElementById('filterTxMethod')?.addEventListener('change', renderAdminTxTable);
  document.getElementById('searchTx')?.addEventListener('input', renderAdminTxTable);
  document.getElementById('filterTxFrom')?.addEventListener('change', renderAdminTxTable);
  document.getElementById('filterTxTo')?.addEventListener('change', renderAdminTxTable);
  document.getElementById('exportTxCSVBtn')?.addEventListener('click', exportTxCSV);
...
async function refreshAllData() {
  try {
    const [statsRes, usersRes, txsRes, notifsRes, actRes] = await Promise.all([
      Admin.stats(),
...
function openUserDetail(userId) {
  ...
  const txs = _txs.filter(t => t.userId === userId);
  const income = txs.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
  const expense = txs.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
  const status = u.isSuspended ? 'suspended' : 'active';

  const el_detailAvatar = document.getElementById('detailAvatar');
  const el_detailName = document.getElementById('detailName');
  const el_detailBiz = document.getElementById('detailBiz');
  const el_detailEmail = document.getElementById('detailEmail');
  const el_detailTxCount = document.getElementById('detailTxCount');
  const el_detailIncome = document.getElementById('detailIncome');
  const el_detailExpense = document.getElementById('detailExpense');
  const el_detailId = document.getElementById('detailId');
  const el_detailJoined = document.getElementById('detailJoined');
  const el_detailBooks = document.getElementById('detailBooks');
  const el_detailCategories = document.getElementById('detailCategories');
  const el_detailStatusPill = document.getElementById('detailStatusPill');

  if (el_detailAvatar) el_detailAvatar.textContent = (u.fullName || '?').charAt(0).toUpperCase();
  if (el_detailName) el_detailName.textContent = u.fullName || '—';
  if (el_detailBiz) el_detailBiz.textContent = u.businessName || '—';
  if (el_detailEmail) el_detailEmail.textContent = u.email || '—';
  if (el_detailTxCount) el_detailTxCount.textContent = txs.length;
  if (el_detailIncome) el_detailIncome.textContent = fmt(income);
  if (el_detailExpense) el_detailExpense.textContent = fmt(expense);
  if (el_detailId) el_detailId.textContent = u._id || u.id;
  if (el_detailJoined) el_detailJoined.textContent = fmtDate(u.createdAt);
  if (el_detailBooks) el_detailBooks.textContent = '—';
  if (el_detailCategories) el_detailCategories.textContent = '—';
  if (el_detailStatusPill) el_detailStatusPill.innerHTML = `<span class="pill ${status}">${status}</span>`;
  ...
  const el_detailRecentTx = document.getElementById('detailRecentTx');
  if (el_detailRecentTx) el_detailRecentTx.innerHTML = recent.length ? recent.map(t => `
    <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--surface-3);font-size:12px;">
      <div>
        <div style="font-weight:600;">${escapeHtml(t.category) || 'Uncategorized'}</div>
        <div style="color:var(--ink-soft);">${fmtDate(t.date)} · ${cap(t.method || 'cash')}</div>
      </div>
      <div style="font-weight:700;color:${t.type === 'income' ? 'var(--green)' : 'var(--red)'};">
        ${t.type === 'income' ? '+' : '-'}${fmt(t.amount)}
      </div>
    </div>
  `).join('') : '<div style="font-size:12px;color:var(--ink-soft);padding:8px 0;">No transactions yet.</div>';
...
function renderAdminTxTable() {
  ...
  const method = document.getElementById('filterTxMethod').value;
  const search = document.getElementById('searchTx').value.toLowerCase();
  const from = document.getElementById('filterTxFrom').value;
  const to = document.getElementById('filterTxTo').value;

  if (type) txs = txs.filter(t => t.type === type);
  if (method) txs = txs.filter(t => t.method === method);
  if (from) txs = txs.filter(t => t.date >= from);
  if (to) txs = txs.filter(t => t.date <= to);
  if (search) {
    txs = txs.filter(t => {
      const u = users.find(u => (u._id || u.id) === t.userId);
  ...
  const totalPages = Math.ceil(total / PAGE_SIZE);
  const paged = txs.slice((txPage - 1) * PAGE_SIZE, txPage * PAGE_SIZE);
  const tbody = document.getElementById('adminTxBody');

  if (paged.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><i class="fa-solid fa-receipt"></i><p>No transactions found.</p></div></td></tr>`;
    renderPagination('txPagination', txPage, totalPages, (p) => { txPage = p; renderAdminTxTable(); });
    return;
  }
  ...
  tbody.innerHTML = paged.map(t => {
    ...
    <tr>
      ...
      <td>
        <div style="font-weight:600;font-size:13px;">${escapeHtml(u?.fullName) || '—'}</div>
      </td>
      <td style="font-size:12px;color:var(--ink-soft);">${escapeHtml(u?.businessName) || '—'}</td>
      <td><span class="pill ${t.type}">${t.type}</span></td>
      <td style="font-size:12px;">${escapeHtml(t.category) || '—'}</td>
      <td style="font-size:12px;">${cap(t.method || 'cash')}</td>
      <td style="font-weight:700;color:${t.type === 'income' ? 'var(--green)' : 'var(--red)'};">
        ${t.type === 'income' ? '+' : '-'}${fmt(t.amount)}
      </td>
      <td style="font-size:12px;color:var(--ink-soft);max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
        ${escapeHtml(t.description || t.desc) || '—'}
      </td>
    </tr>`;
  }).join('');
...
function exportTxCSV() {
  ...
  const headers = ['Date', 'User', 'Business', 'Type', 'Category', 'Method', 'Amount', 'Description'];
  const rows = txs.map(t => {
    const u = users.find(u => (u._id || u.id) === t.userId);
    return [t.date, u?.fullName || '', u?.businessName || '', t.type, t.category || '', t.method || '', t.amount, (t.description || t.desc || '').replace(/,/g, ';')];
  });
  ...
  a.href = url; a.download = 'admin-transactions-' + new Date().toISOString().split('T')[0] + '.csv'; a.click();
...
function showPage(name) {
  ...
  if (name === 'transactions') { txPage = 1; renderAdminTxTable(); }
